...
- **I/O contract:** reads `workspace/flags/` + `workspace/verdicts/` → writes a `situation` to `workspace/situations/{id}.json`. Appends to `workspace/observe/run.jsonl`.
...
> its member aircraft, their flags, and the Investigator's verdicts. Name the emergent situation it
> represents and explain it in one line — e.g. several holds plus go-arounds near KBOS = a possible
> ground stop.
>
