...
export default class extends Controller {
  static targets = ["map", "panel", "banner", "themeIcon", "realtimeButton", "demoButton", "synthesisButton"]
  static values = {
    frame: Object,
    flags: Array,
    verdicts: Array,
  ...
  generateSynthesis() {
    if (this.hasSynthesisButtonTarget) {
      this.synthesisButtonTarget.disabled = true
      this.synthesisButtonTarget.textContent = "Generating..."
    }
    ...
    fetch("/synthesis", {
      ...
      .then((data) => {
        if (data.situation) this.handleSituation(data.situation)
      })
      .catch(() => {})
      .finally(() => {
        if (!this.hasSynthesisButtonTarget) return

        this.synthesisButtonTarget.disabled = false
        this.synthesisButtonTarget.textContent = "Generate"
      })
  }
